...
- (id)init {
	...
	if (self) {
		...
		if (editMenuItem) {
			...
			NSMenuItem *menuItem;
			menuItem = [[NSMenuItem alloc] initWithTitle:@"Uncrustify Selected Files & Groups" action:@selector(uncrustifySelectedFilesAndGroups:) keyEquivalent:@""];
...
- (IBAction)uncrustifySelectedFilesAndGroups:(id)sender {
	int i = 0;
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
	if ([menuItem action] == @selector(uncrustifySelectedFilesAndGroups:)) {
